    ...
    public static void main(String[] args) {
        ...
        /*
         Check the given password by user and validate with following criteria...
            ...
            vi) must contain at least one symbol
         */
...
//        ii) must not contain space characters
        String passwordWithoutSpace = password.replaceAll("[^ ]", ""); // "[^ ]" => means keep the space but remove everything else
...
//        vi) must contain at least one symbol
        boolean symbolCheck = password.replaceAll("[a-zA-Z0-9]", "").length() > 0;
        System.out.println("symbolCheck = " + symbolCheck);


        // In order for this password to be acceptable, ALL above conditions need to pass (true)

        boolean isValid = passwordLength && spaceCheck && upperCaseCheck && lowerCaseCheck && digitCheck && symbolCheck;
        ...
        System.out.println("With ternary = " + passwordAccepted);
